...
object ThreadMapper {
    ...
    fun mapToSections(document: Document): LinkedHashMap<String, List<ForumCategory>> {
    ...
    fun mapToThreads(document: Document): ThreadInfoHolder {
        ...
        for (treadsElem in treadsElems) {
            ...
            if (navonSubj.size > 1) {
                ...
                lastPost = SanitizerMapper.getIntPageFromElem(lastPageElem)
            }
            val pages = SanitizerMapper.getPagesList(navonSubj)
            val sanitizer = UrlQuerySanitizer(treadsElem.select("a").attr("href"))
            val value = sanitizer.getValue("t")
            val topicId = value?.toInt()
            val element = ForumThread(title, lastMsgAuthor, totalMsgs, topicId ?: 0, pPages = pages)
